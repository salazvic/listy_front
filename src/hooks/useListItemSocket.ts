import { listItemSocket } from "@/lib/socket"
...
export function useListItemSocket(idList: string | null) {
  ...
  useEffect(() => {
    const socketListItem = listItemSocket()

    if(!socketListItem) return

    socketListItem.emit(Events.LIST_JOIN, { listId: idList }) 

    socketListItem.on(Events.ITEM_LIST_ADDED, (payload: any) => {
      const newItem = {
        id: payload?.id,
    ...
    socketListItem.on(Events.ITEM_LIST_UPDATED, (payload) => {
      updateItemList(payload?.listId, payload)
    })

    socketListItem.on(Events.ALL_ITEM_LIST_UPDATED, (payload) => {
      setAllPurchased(payload.purchased)
      updateAllItemList(payload?.listId, payload)
    })

    socketListItem.on(Events.ITEM_LIST_DELETED, (payload: any) => {
      deleteItemList(payload.listId, payload.id)
    })
    
    return () => {
      socketListItem.off(Events.LIST_LEFT)
      socketListItem.off(Events.ITEM_LIST_ADDED);
      socketListItem.off(Events.ALL_ITEM_LIST_UPDATED)
      socketListItem.off(Events.ITEM_LIST_UPDATED)
    }     
  },[])
}